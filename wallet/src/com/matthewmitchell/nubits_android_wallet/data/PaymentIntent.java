...
package com.matthewmitchell.nubits_android_wallet.data;
...
public final class PaymentIntent implements Parcelable
{
    ...
    public static PaymentIntent fromNubitsUri(final NubitsURI NubitsUri)
    {
        ...
        final String bluetoothMac = (String) NubitsUri.getParameterByName(Bluetooth.MAC_URI_PARAM);
        final String paymentRequestHashStr = (String) NubitsUri.getParameterByName("h");
        final byte[] paymentRequestHash = paymentRequestHashStr != null ? base64UrlDecode(paymentRequestHashStr) : null;

        return new PaymentIntent(PaymentIntent.Standard.BIP21, null, null, outputs, NubitsUri.getLabel(), bluetoothMac != null ? "bt:"
                + bluetoothMac : null, null, NubitsUri.getPaymentRequestUrl(), paymentRequestHash, address.getParameters());
    }
        ...
        if (i != -1) {
            ...
            if (coin != null) {
                try {
                    ...
                    NubitsURI uri = new NubitsURI(coin, input, coin.getId());
                    return PaymentIntent.fromNubitsUri(uri);
                } catch (NubitsURIParseException ex) {
                }
            }
        }
