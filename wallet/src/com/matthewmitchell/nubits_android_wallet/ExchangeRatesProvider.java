...
public class ExchangeRatesProvider extends ContentProvider
{
	...
	private static final URL USD_URL;
	private static final String USD_SOURCE = "coinmarketcap-nexuist.rhcloud.com";

	// https://bitmarket.eu/api/ticker

	static {
		try {
			USD_URL = new URL("https://coinmarketcap-nexuist.rhcloud.com/api/nbt");
	...
	private static Map<String, ExchangeRate> requestExchangeRates(final String userAgent) {
		...
		String usdPriceResult = getURLResult(USD_URL, userAgent);
		...
		try {
			...
			usdPrice = GenericUtils.parseCoin(jsonResult.getJSONObject("price").getString("usd"), 0);
		}catch (JSONException e) {
			return null;
		}
			
		rates.put("USD", new ExchangeRate("USD", usdPrice, USD_SOURCE));
		...
		if (yahooRes != null) {
			try {
				...
				for (int x = 0; x < convs.length(); x++) {
					...
					BigInteger price = (new BigDecimal(GenericUtils.parseCoin(priceString, 0).multiply(usdPrice))).movePointLeft(4).toBigInteger();
					rates.put(currency, new ExchangeRate(currency, price, USD_SOURCE));
