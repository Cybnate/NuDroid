...
public class ExchangeRatesProvider extends ContentProvider
{
	...
	private static Map<String, WalletExchangeRate> requestExchangeRates(final String userAgent) {
		...
		try {
			...
			String usdStr = jsonResult.getJSONObject("price").getString("usd");
			usdPrice = Fiat.parseFiat("USD", usdStr);
		}catch (JSONException e) {
			return null;
		}
			
		rates.put("USD", new WalletExchangeRate(new ExchangeRate(usdPrice), USD_SOURCE));
		...
		if (yahooRes != null) {
			try {
				...
				for (int x = 0; x < convs.length(); x++) {
					...
					rates.put(currency, new WalletExchangeRate(new ExchangeRate(price), USD_SOURCE));
				}
